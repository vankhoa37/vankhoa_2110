#include "chess_quiz4.hpp"
#include <SDL_mixer.h>
ChessQuiz4::ChessQuiz4() {
...
bool ChessQuiz4::isPositionInsideRect(int x, int y, const SDL_Rect& rect) {
    return (x >= rect.x && x < rect.x + rect.w && y >= rect.y && y < rect.y + rect.h);
}
void ChessQuiz4::handlePieceMovement(int mouseX, int mouseY) {
...
void ChessQuiz4::x(std::string move) {
...
bool ChessQuiz4::isValidMove(int startRow, int startCol, int targetRow, int targetCol) {
    return (targetRow >= 0 && targetRow < 8 && targetCol >= 0 && targetCol < 8 && ((targetRow != startRow) || (targetCol != startCol)));
}
void ChessQuiz4::loadPosition() {
    int k = 0;
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 8; ++j) {
            int n = board[i][j];
...
void ChessQuiz4::run(int valImage) {
    bool quit = false;
    int i = 0;
    int valCorlor = 0;
    int dem = 0;
