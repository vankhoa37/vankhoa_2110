...
ChessQuiz::ChessQuiz() {
    ...
    title = "Get a big advantage with 3 moves!";
...
bool ChessQuiz::isPositionInsideRect(int x, int y, const SDL_Rect& rect) {
    return (x >= rect.x && x < rect.x + rect.w && y >= rect.y && y < rect.y + rect.h);
}
void ChessQuiz::handlePieceMovement(int mouseX, int mouseY) {
...
void ChessQuiz::x(std::string move) {
...
bool ChessQuiz::isValidMove(int startRow, int startCol, int targetRow, int targetCol) {
    return (targetRow >= 0 && targetRow < 8 && targetCol >= 0 && targetCol < 8 && ((targetRow != startRow) || (targetCol != startCol)));
}
void ChessQuiz::loadPosition() {
    int k = 0;
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 8; ++j) {
            int n = board[i][j];
...
void ChessQuiz::run() {
    ...
    while (!quit) {
        ...
        if(title == "Incorrect, Please try again!"  && countFalseMove == 1) {
            SDL_Delay(1000);
            countFalseMove = 0;
        }
        Mix_CloseAudio();
